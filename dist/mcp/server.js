...
import { z } from "zod";
export async function startMcpServer({ config, terminal, logger }) {
    // 检查是否使用 stdio 传输协议
            ...
            if (sessionId) {
                ...
                transport = new StreamableHTTPServerTransport({
                    ...
                    onsessioninitialized: (newSessionId) => {
                        transportBySession[newSessionId] = transport;
                    }
                });
                transport.onclose = () => {
    ...
    logger.info({ host: config.mcpHost, port: config.mcpPort }, "MCP HTTP server listening.");
    return {
        close: async () => {
...
function buildServer(terminal, logger) {
    ...
    server.registerTool("terminal_run_command", {
    ...
    server.registerTool("terminal_get_command_status", {
    ...
    server.registerTool("terminal_capture_output", {
    ...
    server.registerTool("terminal_kill_session", {
    ...
    server.registerTool("filesystem_read_file", {
        description: "Read a file from disk. Paths under ~/.config are blocked to protect credentials and local secrets.",
        inputSchema: z.object({
            path: z.string().min(1),
        ...
        try {
            ...
            logger.warn({ err: error, tool: "filesystem_read_file", path }, "Tool call failed.");
            return {
    ...
    server.registerTool("terminal_list_commands", {
